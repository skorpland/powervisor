defmodule Powervisor.Manager do
  @moduledoc false
  use GenServer, restart: :transient
  require Logger

  alias Powervisor.Protocol.Server
  alias Powervisor.Tenants
  alias Powervisor.Helpers

  @check_timeout 120_000

  def start_link(args) do
    name = {:via, Registry, {Powervisor.Registry.Tenants, {:manager, args.id}}}
  ...
  def init(args) do
    ...
    Registry.register(Powervisor.Registry.ManagerTables, args.id, tid)
  ...
  def handle_call({:subscribe, pid}, _, state) do
    ...
    {reply, new_state} =
      if :ets.info(state.tid, :size) < state.max_clients or Powervisor.mode(state.id) == :session do
