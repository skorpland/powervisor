defmodule Powervisor.HandlerHelpers do
  @moduledoc false
  ...
  @spec try_ssl_handshake(Powervisor.tcp_sock(), boolean) ::
          {:ok, Powervisor.sock()} | {:error, term()}
  ...
  @spec ssl_recv(Powervisor.tcp_sock()) :: {:ok, Powervisor.ssl_sock()} | {:error, term}
  ...
  @spec ssl_connect(Powervisor.tcp_sock(), pos_integer) ::
  ...
  @spec send_error(Powervisor.sock(), String.t(), String.t()) :: :ok | {:error, term()}
  ...
  def listen_cancel_query(pid, key) do
    PubSub.subscribe(Powervisor.PubSub, "cancel_req:#{pid}_#{key}")
  end
  ...
  ## Examples
    ...
    iex> Powervisor.HandlerHelpers.filter_cidrs(["0.0.0.0/0", "::/0"], {127, 0, 0, 1})
    ["0.0.0.0/0"]

    iex> Powervisor.HandlerHelpers.filter_cidrs(["71.209.249.38/32"], {71, 209, 249, 39})
    []

    iex> Powervisor.HandlerHelpers.filter_cidrs(["0.0.0.0/0", "::/0"], {8193, 3512, 34211, 0, 0, 35374, 880, 29492})
    ["::/0"]

    iex> Powervisor.HandlerHelpers.filter_cidrs(["0.0.0.0/0", "::/0"], :error)
    []
  ...
  @spec addr_from_sock(Powervisor.sock()) :: {:ok, :inet.ip_address()} | :error
