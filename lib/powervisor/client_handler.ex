defmodule Powervisor.ClientHandler do
  @moduledoc """
  This module is responsible for handling incoming connections to the Powervisor server. It is
  ...
  @subscribe_retries Application.compile_env(:powervisor, :subscribe_retries)
  @timeout_subscribe 500
  @clients_registry Powervisor.Registry.TenantClients
  @proxy_clients_registry Powervisor.Registry.TenantProxyClients
  ...
  require Powervisor.Protocol.Server, as: Server
  ...
  def handle_event(:info, {_proto, _, <<"GET", _::binary>>}, :exchange, data) do
    ...
    HandlerHelpers.sock_send(
      data.sock,
      "HTTP/1.1 204 OK\r\nx-app-version: #{Application.spec(:powervisor, :vsn)}\r\n\r\n"
    )
    ...
    case handle_exchange(sock, {method, secrets}) do
      {:error, reason} ->
        ...
        if method != :password and reason == "Wrong password" and
             Cachex.get(Powervisor.Cache, key) == {:ok, nil} do
  ...
  def handle_event(:internal, :subscribe, _, data) do
    ...
    with {:ok, sup} <-
           Powervisor.start_dist(data.id, data.auth_secrets,
         ...
         {:ok, opts} <- Powervisor.subscribe(sup, data.id) do
      ...
      :proxy ->
        case Powervisor.get_pool_ranch(data.id) do
  ...
  @spec handle_exchange(Powervisor.sock(), {atom(), fun()}) ::
  ...
  defp update_user_data(data, info, user, id, db_name, mode) do
    ...
    auth = %{
      application_name: data[:app_name] || "Powervisor",
      database: db_name,
  ...
  @spec auth_secrets(map, String.t(), term(), non_neg_integer()) ::
          {:ok, Powervisor.secrets()} | {:error, term()}
  ## password secrets
  ...
  def auth_secrets(info, db_user, key, ttl) do
    ...
    case Cachex.fetch(Powervisor.Cache, key, fetch) do
  ...
  def get_secrets(%{user: user, tenant: tenant}, db_user) do
    ...
    {:ok, conn} =
      Postgrex.start_link(
        ...
        parameters: [application_name: "Powervisor auth_query"],
  ...
  @spec try_get_sni(Powervisor.sock()) :: String.t() | nil
  ...
  defp db_pid_meta({_, {_, pid, _}} = _key) do
    rkey = Powervisor.Registry.PoolPids
    fnode = node(pid)
  ...
  defp handle_data(:info, bin, :idle, data) do
    query_type =
      with {:ok, payload} <- Client.get_payload(bin),
           {:ok, statements} <- Powervisor.PgParser.statements(payload) do
        Logger.debug(
  ...
  @spec handle_prepared_statements({pid, pid, Powervisor.sock()}, binary, map) :: :ok | nil
  defp handle_prepared_statements({_, pid, _}, bin, %{mode: :transaction} = data) do
    with {:ok, payload} <- Client.get_payload(bin),
         {:ok, statements} <- Powervisor.PgParser.statements(payload),
      ...
      |> Enum.each(fn
        {_, ^pid, _, [Powervisor.DbHandler]} ->
          Logger.debug("ClientHandler: Linked DbHandler #{inspect(pid)}")
          nil

        {_, pool_proc, _, [Powervisor.DbHandler]} ->
          Logger.debug(
