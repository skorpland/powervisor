defmodule Powervisor.HotUpgrade do
  @moduledoc false
  require Logger
  ...
  def down(_app, from_vsn, _to_vsn, appup, _transform) do
    [
      {:apply, {Powervisor.HotUpgrade, :apply_runtime_config, [from_vsn]}},
      {:apply, {__MODULE__, :reint_funs, []}}
    ] ++ appup
  end
  ...
  def apply_runtime_config(vsn) do
    ...
    if File.exists?(path) do
      ...
      for {app, config} <-
            Config.Reader.read!(path, env: Application.get_env(:powervisor, :env)) do
        updated_config =
          Config.Reader.merge(
  ...
  def reinit_pool_args do
    for [_tenant, pid, _meta] <-
          Registry.select(Powervisor.Registry.TenantSups, [
            {{:"$1", :"$2", :"$3"}, [], [[:"$1", :"$2", :"$3"]]}
          ]),
      ...
      :sys.replace_state(linked_pid, fn state ->
        db_handler = Powervisor.DbHandler
    ...
    |> Enum.each(fn entry(key: key, value: value) ->
      case value do
        {:cached, {:ok, {:auth_query, auth}}} when is_function(auth) ->
          ...
          Cachex.put(Powervisor.Cache, key, new)
