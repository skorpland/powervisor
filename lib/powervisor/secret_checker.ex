defmodule Powervisor.SecretChecker do
  @moduledoc false
  ...
  def start_link(args) do
    name = {:via, Registry, {Powervisor.Registry.Tenants, {:secret_checker, args.id}}}
  ...
  def init(args) do
    ...
    tenant = Powervisor.tenant(args.id)
  ...
  def check_secrets(%{auth: auth, user: user, conn: conn} = state) do
    case Helpers.get_user_secret(conn, auth.auth_query, user) do
      {:ok, secret} ->
        ...
        update_cache =
          case Cachex.get(Powervisor.Cache, state.key) do
        ...
        if update_cache do
          ...
          Cachex.put(Powervisor.Cache, state.key, {:cached, value}, expire: :timer.hours(24))
        end
