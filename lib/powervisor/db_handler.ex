defmodule Powervisor.DbHandler do
  @moduledoc """
  ...
  It uses the Powervisor.Protocol.Server module to decode messages from the database and sends messages to clients Powervisor.ClientHandler.
  """
  ...
  @proto [:tcp, :ssl]
  @switch_active_count Application.compile_env(:powervisor, :switch_active_count)
  @reconnect_retries Application.compile_env(:powervisor, :reconnect_retries)
  ...
  @spec get_state_and_mode(pid()) :: {:ok, {state, Powervisor.mode()}} | {:error, term()}
    ...
    case :gen_tcp.connect(auth.host, auth.port, sock_opts) do
      {:ok, sock} ->
        ...
        case try_ssl_handshake({:gen_tcp, sock}, auth) do
          {:ok, sock} ->
            tenant = if data.proxy, do: Powervisor.tenant(data.id)
            search_path = Powervisor.search_path(data.id)
    ...
    case resp do
      ...
      {:ready_for_query, acc} ->
        ...
        if data.proxy do
          ...
          Powervisor.set_parameter_status(data.id, ps)
        end
  ...
  def handle_event(_, {closed, _}, state, data) when closed in @sock_closed do
    ...
    if Application.get_env(:powervisor, :reconnect_on_db_close),
  ...
  @spec try_ssl_handshake(Powervisor.tcp_sock(), map) :: {:ok, Powervisor.sock()} | {:error, term()}
  ...
  @spec ssl_recv(Powervisor.tcp_sock(), map) :: {:ok, Powervisor.ssl_sock()} | {:error, term}
  ...
  @spec ssl_connect(Powervisor.tcp_sock(), map, pos_integer) ::
          {:ok, Powervisor.ssl_sock()} | {:error, term}
  ...
  @spec send_startup(Powervisor.sock(), map(), String.t() | nil, String.t() | nil) ::
          :ok | {:error, term}
  ...
  @spec sock_send(Powervisor.sock(), iodata) :: :ok | {:error, term}
  defp sock_send({mod, sock}, data) do
    mod.send(sock, data)
  end

  @spec activate(Powervisor.sock()) :: :ok | {:error, term}
  ...
  defp handle_auth_pkts(%{tag: :backend_key_data, payload: payload}, acc, data) do
    ...
    Registry.register(Powervisor.Registry.PoolPids, key, Map.merge(payload, conn))
  ...
  defp handle_authentication_error(%{proxy: false} = data, reason) do
    tenant = Powervisor.tenant(data.id)

    :erpc.multicast([node() | Node.list()], fn ->
      Cachex.del(Powervisor.Cache, {:secrets, tenant, data.user})
      Cachex.del(Powervisor.Cache, {:secrets_check, tenant, data.user})

      Registry.dispatch(Powervisor.Registry.TenantClients, data.id, fn entries ->
        for {client_handler, _meta} <- entries,
            do: send(client_handler, {:disconnect, reason})
      end)
    end)

    Powervisor.stop(data.id)
  end
