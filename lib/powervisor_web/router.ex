defmodule PowervisorWeb.Router do
  use PowervisorWeb, :router

  pipeline :browser do
    plug(:accepts, ["html"])
    plug(:fetch_session)
    plug(:fetch_live_flash)
    plug(:put_root_layout, {PowervisorWeb.LayoutView, :root})
  ...
  pipeline :openapi do
    plug(OpenApiSpex.Plug.PutApiSpec, module: PowervisorWeb.ApiSpec)
  end
  ...
  # websocket pg proxy
  scope "/v2" do
    get("/", PowervisorWeb.WsProxy, [])
  end

  scope "/api", PowervisorWeb do
    pipe_through(:api)
  ...
  scope "/metrics", PowervisorWeb do
  ...
  if Mix.env() in [:dev, :test] do
    ...
    scope "/" do
      ...
      live_dashboard("/dashboard", metrics: PowervisorWeb.Telemetry)
    end
  end
  ...
  defp check_auth(conn, [secret_key, blocklist_key]) do
    secret = Application.fetch_env!(:powervisor, secret_key)
    blocklist = Application.fetch_env!(:powervisor, blocklist_key)

    with ["Bearer " <> token] <- get_req_header(conn, "authorization"),
         token <- Regex.replace(~r/\s|\n/, URI.decode(token), ""),
         false <- token in blocklist,
         {:ok, _claims} <- Powervisor.Jwt.authorize(token, secret) do
      conn
    else
      _ ->
        conn
