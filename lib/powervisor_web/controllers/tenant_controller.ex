defmodule PowervisorWeb.TenantController do
  ...
  alias PowervisorWeb.OpenApiSchemas.{
    Created,
    Empty,
    NotFound,
    Tenant,
  ...
  action_fallback(PowervisorWeb.FallbackController)
  ...
  def update(conn, %{"external_id" => id, "tenant" => params}) do
    Logger.info("Delete cache dist #{id}: #{inspect(Powervisor.del_all_cache_dist(id))}")
      ...
      case Tenants.get_tenant_by_external_id(id) do
        ...
        tenant ->
          ...
          with {:ok, %TenantModel{} = tenant} <-
            ...
            result = Powervisor.terminate_global(tenant.external_id)
  ...
  def terminate(conn, %{"external_id" => external_id}) do
    ...
    result = Powervisor.terminate_global(external_id) |> inspect()
    Logger.warning("Terminate #{external_id}: #{result}")

    Logger.info(
      "Delete cache dist #{external_id}: #{inspect(Powervisor.del_all_cache_dist(external_id))}"
    )
