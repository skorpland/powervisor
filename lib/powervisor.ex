defmodule Powervisor do
  ...
  @registry Powervisor.Registry.Tenants
  @max_pools Application.compile_env(:powervisor, :max_pools, 20)
  ...
  def dirty_terminate(tenant, timeout \\ 15_000) do
    Registry.lookup(Powervisor.Registry.TenantSups, tenant)
  ...
  def terminate_global(tenant) do
    :erpc.multicall([node() | Node.list()], Powervisor, :dirty_terminate, [tenant], 60_000)
  end
  ...
  def del_all_cache(tenant, user) do
    ...
    {:ok, keys} = Cachex.keys(Powervisor.Cache)

    del = fn key, acc ->
      result = Cachex.del(Powervisor.Cache, key)
  ...
  def del_all_cache_dist(tenant, timeout \\ 15_000) do
    ...
    for node <- [node() | Node.list()] do
      %{to_string(node) => :erpc.call(node, Powervisor, :del_all_cache, [tenant], timeout)}
    end
  end
    ...
    |> case do
      [_ | _] = replicas ->
        ...
        DynamicSupervisor.start_child(
          {:via, PartitionSupervisor, {Powervisor.DynamicSupervisor, id}},
          {Powervisor.TenantSupervisor, %{id: id, replicas: opts, log_level: log_level}}
        )
        |> case do
  ...
  def count_pools(tenant),
    do: Registry.count_match(Powervisor.Registry.TenantSups, tenant, :_)
end
