...
config :powervisor, PowervisorWeb.Endpoint,
  server: true,
  http: [
  ...
  if System.get_env("DNS_POLL") do
    dns_poll = [
      ...
      config: [
        ...
        node_basename:
          System.get_env("NODE_NAME") || System.get_env("FLY_APP_NAME") || "powervisor"
      ]
    ]
...
topologies =
  if System.get_env("CLUSTER_POSTGRES") && Application.spec(:powervisor, :vsn) do
    %Version{major: maj, minor: min} =
      Application.spec(:powervisor, :vsn) |> List.to_string() |> Version.parse!()
    ...
    postgres = [
      ...
      config: [
        ...
        channel_name: "powervisor_#{region}_#{maj}_#{min}"
      ]
    ]
...
if config_env() != :test do
  ...
  config :powervisor, Powervisor.Repo,
    url: System.get_env("DATABASE_URL", "ecto://postgres:postgres@localhost:6432/postgres"),
    ...
    parameters: [
      application_name: "powervisor_meta"
    ]

  config :powervisor, Powervisor.Vault,
    ciphers: [
      default: {
