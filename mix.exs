defmodule Powervisor.MixProject do
  use Mix.Project
  ...
  defp aliases do
    [
      ...
      test: [
        ...
        "ecto.migrate --prefix _powervisor --log-migrator-sql",
  ...
  defp upgrade(release) do
    ...
    if from && from != "" do
      vsn = release.version
      path = Path.join([release.path, "releases", "powervisor-#{vsn}.rel"])
      rel_content = File.read!(Path.join(release.version_path, "powervisor.rel"))

      Mix.Task.run("powervisor.gen.appup", ["--from=" <> from, "--to=" <> vsn])
      :ok = File.write!(path, rel_content)
      Mix.Task.run("powervisor.gen.relup", ["--from=" <> from, "--to=" <> vsn])
    end
