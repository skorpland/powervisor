...
# Powervisor - Postgres connection pooler
...
Powervisor is a scalable, cloud-native Postgres connection pooler. A Powervisor
...
For database managers, Powervisor simplifies the task of managing Postgres
clusters by providing easy configuration of highly available Postgres clusters
([todo](#future-work)).

## Motivation

We have several goals with Powervisor:
...
Powervisor was designed to work in a cloud computing environment as a highly
available cluster of nodes. Tenant configuration is stored in a highly available
Postgres database. Configuration is loaded from the Powervisor database when a
tenant connection pool is initiated.

Connection pools are dynamic. When a tenant client connects to the Powervisor
...
connection pool should be alive in a Powervisor cluster. In the case of two
...
- [Installation and usage](https://powerbase.github.io/powervisor/development/installation/)
- [Metrics](https://powerbase.github.io/powervisor/monitoring/metrics/)
...
- Highly available
  - When deployed as a Powervisor cluster and a node dies connection pools should
...
- Config as code
  - Not only for the Powervisor cluster but tenant databases and tenant database
    clusters as well
...
- Running `pgbench` on `Powervisor` (pool size 60, no logs)
...
![Powervisor load test virtual users chart](./docs/images/load-test-vus.png)

![Powervisor load test qps chart](./docs/images/load-test-qps.png)

- Powervisor two node cluster
  - 64vCPU / 246RAM
...
the design decisions for Powervisor.
