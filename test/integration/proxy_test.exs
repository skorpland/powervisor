defmodule Powervisor.Integration.ProxyTest do
  use Powervisor.DataCase, async: false
  ...
  setup do
    db_conf = Application.get_env(:powervisor, Repo)

    assert {:ok, proxy} =
             Postgrex.start_link(
               hostname: db_conf[:hostname],
               port: Application.get_env(:powervisor, :proxy_port_transaction),
  ...
  test "prepared statement", %{proxy: proxy} do
    prepare_sql =
      "PREPARE tenant (text) AS SELECT id, external_id FROM _powervisor.tenants WHERE external_id = $1;"

    db_conf = Application.get_env(:powervisor, Repo)

    assert {:ok, pid} =
             Keyword.merge(db_conf, username: db_conf[:username] <> "." <> @tenant)
             |> single_connection(Application.get_env(:powervisor, :proxy_port_transaction))
  ...
  test "the wrong password" do
    ...
    url =
      "postgresql://#{db_conf[:username] <> "." <> @tenant}:no_pass@#{db_conf[:hostname]}:#{Application.get_env(:powervisor, :proxy_port_transaction)}/postgres"
  ...
  test "query via another node", %{proxy: proxy, user: user} do
    ...
    sup =
      Enum.reduce_while(1..30, nil, fn _, acc ->
        case Powervisor.get_global_sup({@tenant, user, :transaction}) do
          nil ->
            Process.sleep(100)
            {:cont, acc}
    ...
    db_conf = Application.fetch_env!(:powervisor, Repo)

    assert {:ok, proxy2} =
             Postgrex.start_link(
               hostname: db_conf[:hostname],
               port: Application.get_env(:powervisor, :secondary_proxy_port),
               database: db_conf[:database],
               password: db_conf[:password],
               username: db_conf[:username] <> "." <> @tenant
             )
  ...
  test "too many clients in session mode" do
    db_conf = Application.get_env(:powervisor, Repo)
    port = Application.get_env(:powervisor, :proxy_port_session)
  ...
  test "http to proxy server returns 200 OK" do
    assert :httpc.request(
             "http://localhost:#{Application.get_env(:powervisor, :proxy_port_transaction)}"
           ) ==
             {:ok,
              {{~c"HTTP/1.1", 204, ~c"OK"},
               [{~c"x-app-version", Application.spec(:powervisor, :vsn)}], []}}
  end
  ...
  test "checks that client_handler is idle and db_pid is nil for transaction mode" do
    ...
    url =
      "postgresql://transaction.#{@tenant}:#{db_conf[:password]}@#{db_conf[:hostname]}:#{Application.get_env(:powervisor, :proxy_port_transaction)}/postgres"

    assert {:ok, pid} = parse_uri(url) |> single_connection()

    [{_, client_pid, _}] =
      Powervisor.get_local_manager(
  ...
  test "change role password", %{origin: origin} do
    ...
    conn = fn pass ->
      "postgresql://dev_postgres.is_manager:#{pass}@#{db_conf[:hostname]}:#{Application.get_env(:powervisor, :proxy_port_transaction)}/postgres?sslmode=disable"
    end
    ...
    Powervisor.stop({{:single, "is_manager"}, "dev_postgres", :transaction, "postgres"})
  ...
  test "invalid characters in user or db_name" do
    ...
    url =
      "postgresql://user\x10user.#{@tenant}:#{db_conf[:password]}@#{db_conf[:hostname]}:#{Application.get_env(:powervisor, :proxy_port_transaction)}/postgres\\\\\\\\\"\\"
  ...
  test "active_count doesn't block" do
    ...
    [{client_pid, _}] = Registry.lookup(Powervisor.Registry.TenantClients, id)
