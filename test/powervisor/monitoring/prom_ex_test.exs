defmodule Powervisor.Monitoring.PromExTest do
  use Powervisor.DataCase, async: true
  use ExUnitProperties

  @subject Powervisor.Monitoring.PromEx
  ...
  describe "get_metrics/1" do
    ...
    setup do
      {:ok, prom2json: Powervisor.Downloader.ensure("prom2json", @sources)}
    end
    ...
    property "non-standard DB names do not cause parsing issues", %{tmp_dir: dir, prom2json: exe} do
      ...
      check all db_name <- string(:printable, min_length: 1, max_length: 63) do
        Powervisor.Monitoring.Telem.client_join(
          :ok,
        ...
        assert %{"metrics" => metrics} =
                 Enum.find(measurements, &(&1["name"] == "powervisor_client_joins_ok"))
