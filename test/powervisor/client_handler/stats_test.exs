defmodule Powervisor.ClientHandler.StatsTest do
  use Powervisor.E2ECase, async: false
  ...
  setup ctx do
    ...
    :telemetry.attach(
      {ctx.test, :client},
      [:powervisor, :client, :network, :stat],
  ...
  def handle_event([:powervisor, name, :network, :stat], measurement, meta, {pid, ref}) do
  ...
  setup ctx do
    conn =
      start_supervised!(
        {SingleConnection,
         hostname: "localhost",
         port: Application.fetch_env!(:powervisor, :proxy_port_transaction),
         database: ctx.db,
         username: ctx.user,
         password: "postgres"}
      )
  ...
  describe "client network usage" do
    ...
    test "another instance do not send events here", %{telemetry: telemetry} = ctx do
      assert {:ok, _pid, node} = Powervisor.Support.Cluster.start_node()
      ...
      other_conn =
        start_supervised!(
          {SingleConnection,
           hostname: "localhost",
           port: Application.fetch_env!(:powervisor, :secondary_proxy_port),
           database: ctx.db,
           username: ctx.user,
           password: "postgres"},
          id: :postgrex_another
