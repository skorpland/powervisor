defmodule PowervisorWeb.TenantControllerTest do
  use PowervisorWeb.ConnCase, async: false

  import Powervisor.TenantsFixtures
  ...
  setup %{conn: conn} do
    :meck.expect(Powervisor.Helpers, :check_creds_get_ver, fn _ -> {:ok, "0.0"} end)
    ...
    on_exit(fn ->
      :meck.unload(Powervisor.Helpers)
    end)
  ...
  describe "update tenant" do
    ...
    test "triggers Powervisor.stop/2", %{
      conn: conn,
  ...
  defp set_cache(external_id) do
    Powervisor.Tenants.get_user_cache(:single, "user", external_id, nil)
    Powervisor.Tenants.get_tenant_cache(external_id, nil)
  end

  defp check_cache(external_id) do
    assert {:ok, nil} =
             Cachex.get(Powervisor.Cache, {:user_cache, :single, "user", external_id, nil})

    assert {:ok, nil} = Cachex.get(Powervisor.Cache, {:tenant_cache, external_id, nil})
  end
end
